...
public class UpdateHandler : IUpdateHandler
{
    ...
    private static readonly Html2JsonTransformParameters TransformParameters;
    ...
    static UpdateHandler()
    {
        ...
        TransformParameters = new Html2JsonTransformParameters
        {
    ...
    private async Task BotOnMessageReceived(Message message, CancellationToken cancellationToken)
    {
        ...
        if (message is { Type: MessageType.Document, Document: not null })
        {
        ...
        // Отправка пользователю информационного сообщения
            ...
            CancellationToken cancellationToken)
        {
            ...
            return await SendTextMessageAsyncEx(botClient, message, responseText, cancellationToken);
            ...
            //    replyMarkup: new ReplyKeyboardRemove(),
            //    cancellationToken: cancellationToken,
            //    replyToMessageId: null); (message.Type == MessageType.Text) ? message.MessageId : null);
        }
...
#pragma warning disable CS8602
            await botClient.GetInfoAndDownloadFileAsync(message.Document.FileId, msRead,
#pragma warning restore CS8602
            ...
            try
            {
                var resultData = new Html2JsonTransformer().Transform(fileData, TransformParameters);
                ...
                else // Файл успешно обработан, записываем результат в файл и отправляем его клиенту
                {
                    ...
                    var sendMessageTask = await SendDocumentAsyncEx(botClient, message, resultInfo,  new InputFileStream(new MemoryStream(fileBytes), fileName), cancellationToken);
        ...
        return await SendTextMessageAsyncEx(botClient, message, resultInfo, cancellationToken);
    ...
    private static async Task<Message> SendTextMessageAsyncEx(ITelegramBotClient botClient, Message message,
    ...
    private static async Task<Message> SendDocumentAsyncEx(ITelegramBotClient botClient, Message message,
        string caption, InputFile fileStream, CancellationToken cancellationToken)
    {
        try
        {
